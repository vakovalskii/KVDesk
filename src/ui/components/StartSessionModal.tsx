...
import { usePromptActions } from "./PromptInput";
...
export function StartSessionModal({
  ...
  const displayModel = (() => {
    ...
    return t("startSessionModal.selectModel");
  })();
    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-ink-900/20 px-4 py-8 backdrop-blur-sm">
      <div className="w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-2xl border border-ink-900/5 bg-surface p-6 shadow-elevated">
        <div className="flex items-center justify-between">
          ...
          <button className="rounded-full p-1.5 text-muted hover:bg-surface-tertiary hover:text-ink-700 transition-colors" onClick={onClose} aria-label={t("startSessionModal.close")}>
            <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M18 6L6 18M6 6l12 12" />
            </svg>
          </button>
        </div>
        <p className="mt-2 text-sm text-muted">{t("startSessionModal.description")}</p>
        <div className="mt-5 grid gap-4">
          <label className="grid gap-1.5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium text-muted">{t("startSessionModal.model")}</span>
            </div>
            <DropdownMenu.Root>
              ...
              <DropdownMenu.Portal>
                <DropdownMenu.Content className="z-50 min-w-[300px] max-w-[400px] rounded-xl border border-ink-900/10 bg-white shadow-lg" sideOffset={8}>
                  ...
                  <div className="p-2 border-b border-ink-900/10">
                    <input
                      type="text"
                      placeholder={t("startSessionModal.searchModels")}
                      value={modelSearch}
                  ...
                  <div className="max-h-60 overflow-y-auto p-1">
                    {allAvailableModels.length === 0 ? (
                      <div className="px-3 py-2 text-sm text-muted">{t("startSessionModal.noModelsAvailable")}</div>
                    ) : filteredModels.length === 0 ? (
                      <div className="px-3 py-2 text-sm text-muted">{t("startSessionModal.noModelsFound", { query: modelSearch })}</div>
                    ) : (
              ...
              <div className="flex items-center justify-between mt-1">
                <span className="text-[10px] text-muted">
                  {schedulerDefaultModel === (allAvailableModels.find(m => m.id === selectedModel)?.name ?? selectedModel)
                    ? t("startSessionModal.defaultForScheduledTasks")
                    : ""}
                </span>
                {schedulerDefaultModel !== (allAvailableModels.find(m => m.id === selectedModel)?.name ?? selectedModel) && (
                  <button
                  ...
                  >
                    {t("startSessionModal.setDefaultForTasks")}
                  </button>
                )}
              </div>
            )}
          ...
          <div className="grid gap-1.5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="text-xs font-medium text-muted">{t("startSessionModal.temperature")}</span>
                <label className="flex items-center gap-1 cursor-pointer">
                  ...
                  />
                  <span className="text-[10px] text-muted">{t("startSessionModal.send")}</span>
                </label>
              </div>
            ...
            <p className="text-[10px] text-muted-light">
              {t("startSessionModal.temperatureDesc")}
            </p>
          </div>

          <label className="grid gap-1.5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium text-muted">{t("startSessionModal.workspaceFolder")}</span>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder={t("startSessionModal.workspacePlaceholder")}
                value={cwd}
              ...
              >
                {t("startSessionModal.browse")}
              </button>
            </div>
            <p className="text-[11px] text-muted-light">
              {t("startSessionModal.workspaceTip")}
            </p>
            {recentCwds.length > 0 && (
              <div className="mt-2 grid gap-2 w-full">
                <div className="text-[11px] font-medium uppercase tracking-wide text-muted-light">{t("startSessionModal.recent")}</div>
          ...
          <label className="grid gap-1.5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium text-muted">{t("startSessionModal.initialMessage")}</span>
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-ink-100 text-ink-600 font-medium">{t("startSessionModal.optional")}</span>
            </div>
            <textarea
              rows={4}
              ...
              placeholder={t("startSessionModal.initialMessagePlaceholder")}
              value={prompt}
...
type StartSessionModalWithActionsProps = Omit<StartSessionModalProps, "onStart"> & {
  sendEvent: (event: ClientEvent) => void;
};

/** Обёртка, предоставляющая handleStartFromModal из usePromptActions (требует I18nProvider) */
export function StartSessionModalWithActions({
  sendEvent,
  ...rest
}: StartSessionModalWithActionsProps) {
  const { handleStartFromModal } = usePromptActions(sendEvent);
  return <StartSessionModal {...rest} onStart={handleStartFromModal} />;
}
